...
RSpec.describe 'Arrays' do
  ...
  it 'has handy useful methods to operator on all of an array' do
    ...
    expect(array.respond_to?(:each)).to eq(true)
    expect(array.respond_to?(:all?)).to eq(true)
    expect(array.respond_to?(:map)).to eq(true)
    expect(array.respond_to?(:select)).to eq(true)
  end
end
